...
Precondition: linked list pointer assigned to null or to one op command line,
...
Algorithm: recursively iterated through op code linked list,
...
bool getMetaData( char *fileName,
    ...
    *opCodeDataHead = NULL;
    ...
    // loop across ll remaining op commands
    ...
    while( accessResult == COMPLETE_OPCMD_FOUND_MSG )
        ...
        // update start and end counts for later comparison
            // function: updateStartCount, updateEndCounr
        startCount = updateStartCount( startCount, newNodePtr->strArg1 );
        endCount = updateEndCount( endCount, newNodePtr->strArg1 );
    ...
    // assign temporary local local head pointer to parameter return pointer
...
Postcondition: if op string has "end", input count + 1 is returned;
               otherwise, inout count is returned unchanged
Exceptions: none
Notes: none
*/
