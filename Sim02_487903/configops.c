...
ConfigDataType *clearConfigData( ConfigDataType *configData )
    {
        // check that config data is not null
        if( configData != NULL )
            {
...
Postcondition: string parameter hols correct string
...
Precondition: for correct operation, file is available, is formatted correctly,
              and hass all configuration lines and data although
...
bool getConfigData( char *fileName,
    ...
    *configData = NULL;
    ...
    if( getStringToDelimiter( fileAccessPtr, COLON, dataBuffer ) != NO_ERR
        ...
        if( getStringToDelimiter( fileAccessPtr, COLON, dataBuffer ) != NO_ERR )
        {
            printf(" The getStringToDelimiter is equaling Error \n");
        }

        if( compareString( dataBuffer, "Start Simulator Configuration File" ) != STR_EQ)
        {
            printf(" dataBuffer and the sentence are not equal");
        }
        */
    ...
    while( lineCtr < NUM_DATA_LINES )
        ...
        if( getStringToDelimiter( fileAccessPtr, COLON, dataBuffer ) != NO_ERR)
            ...
            // set end state mesage to line capture failure
                // function: copyString
        ...
        if( dataLineCode != CFG_CORRUPT_PROMPT_ERR )
            ...
            // get data value
                ...
                // or CPU schedulign names (strings)
            ...
            // otherwise, assume data value not in range
                ...
                // set endStateMsg to configuration data out of range
                    // function: copyString
        ...
        // otherwise, assume data line not found
            ...
            // set endStateMsg to configuration corrupt prompt error
                // function: copyString
    ...
    // acquire end of sim config string
        // function: getStringToDelimiter
    if( getStringToDelimiter( fileAccessPtr, PERIOD, dataBuffer ) != NO_ERR
        ...
        // set endStateMsg to corrupt configuration end line
            // function: copyString
...
Algorithm: converts string data (e.g., "File", "Monitor")
            to constant code number to be stored as integer
Precondition: codeStr is c-style string with one 
...
Postcondition: returns True if data is within specified parameters,
               Flase otherwise
Exceptions: metadata or logfile names are ignored and return Ture
Notes: none
*/
bool valueInRange( int lineCode, int intVal, double doubleVal, char *stringVal )
    ...
    switch( lineCode )
        ...
        case CFG_CPU_SCHED_CODE:
            ...
            // check for not finding one of the scheduline strings
                // function: compareString
        ...
        case CFG_LOG_TO_CODE:
            ...
            // create temporary string and set to lower case
                // function: getStringLength, malloc, setStrLowerToCase
            strLen = getStringLength( stringVal );
            tempStr = (char *)malloc( strLen+1 );
            setStrToLowerCase( tempStr, stringVal );
